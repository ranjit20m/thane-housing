...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();     
    // const res = await fetch('/api/auth/signup', formData) => Not secure stringfy it
    try {
      setLoading(true);
      ...
      setLoading(false);
      setError(null);
      navigate('/sign-in'); // if everything ok navigate to sign in page
    } catch (error) {
      setLoading(false);
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <div className='flex gap-2 mt-5'>
        <p>Have an account?</p>
        <Link to={'/sign-in'}><span className='text-blue-700'>Sing in</span></Link>
      </div>
